...
void GRAFOdijkstra(Grafo *g, int s, int *pai, int *dp) {
    FilaPriMin *q = FILAPRI_MINconstroi(g->num_vertices);
    Item w;
    ...
    for (int i = 0; i < g->num_vertices; i++) {
        w = ITEM(i, dp[i]);
        FILAPRI_MINinsere(q, w);
    }

    while (!FILAPRI_MINvazia(q)) {
        w = FILAPRI_MINremove(q);

        if (w.chave != INT_MAX) {
            ElemListaAdj *u = g->listas_adj[w.ind];
            
            while (u != NULL) {
                if (dp[u->v] > w.chave + u->p) {
                    dp[u->v] = w.chave + u->p;
                    FILAPRI_MINdiminui_chave(q, u->v, dp[u->v]);
                    pai[u->v] = w.ind;
                }
    ...
    FILAPRI_MINdestroi(q);
}

void GRAFOdestinos_viaveis(Grafo *g, int s, int m) {
    int *pai = malloc(g->num_vertices * sizeof(int));
    int *dp = malloc(g->num_vertices * sizeof(int));

    GRAFOdijkstra(g, s, pai, dp);

    // Prints
  for (int i = 0; i < g->num_vertices; i++) {
    if (dp[i] == INT_MAX || dp[i] > m) {
